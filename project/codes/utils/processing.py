...
def preproc(signal, fc, fs_new, fs, order=64, Q_factor=30, freq=50):
    # Design the filter
    normalized_cutoff = fs_new / fs
    sos = butter(order, normalized_cutoff, btype='low', output='sos')

    # Design the notch filter
    w0 = freq / (fs_new / 2)  # Normalize frequency to Nyquist
    b, a = iirnotch(w0, Q_factor)
    sos_notch = tf2sos(b, a)

    # Prepare to store the downsampled signal
    num_samples_new = int(signal.shape[1] * fs_new / fs)  # New time dimension
    ...
    for ch in range(nChannels):
        ...
        lpf_signal = signal[ch] - lp_filter_zero_phase(signal[ch], fc/fs)
